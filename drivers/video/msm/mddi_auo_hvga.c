/* drivers/video/msm/mddi_auo_hvga.c
 *
...
static int auo_display_on = FALSE;
...
static int mddi_auo_lcd_on(struct platform_device *pdev)
{
	...
	display_table(init_table, size);
		display_table(mddi_auo_display_on,
					  ARRAY_SIZE(mddi_auo_display_on));
...
{
	if(auo_display_on) {
		display_table(mddi_auo_sleep_mode_on_data,
					  ARRAY_SIZE(mddi_auo_sleep_mode_on_data));
	mddi_auo_lcd_panel_poweroff();
		auo_display_on = FALSE;
	}
	return 0;
}
...
ssize_t mddi_auo_lcd_store_onoff(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(onoff) {
		mddi_auo_lcd_on(NULL);
	}
	else {
		mddi_auo_lcd_off(NULL);
	}
...
{
	display_table(mddi_auo_position_table, ARRAY_SIZE(mddi_auo_position_table));
	return 0;
}
EXPORT_SYMBOL(mddi_auo_position);

DEVICE_ATTR(lcd_onoff, 0664, mddi_auo_lcd_show_onoff, mddi_auo_lcd_store_onoff);

// LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110419
static void mddi_auo_lcd_set_window (int x, int y, int xres, int yres)
{
	mddi_auo_position();
}
...
struct msm_fb_panel_data auo_panel_data0 = {
	...
	.set_rect = mddi_auo_lcd_set_window,// LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110419
};
...
static int __init mddi_auo_lcd_probe(struct platform_device *pdev)
{
	int ret;
	EPRINTK("%s: started.\n", __func__);
...
{
	struct msm_panel_hitachi_pdata *pdata = mddi_auo_pdata;
