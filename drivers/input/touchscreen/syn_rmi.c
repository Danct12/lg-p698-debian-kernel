...
#include <mach/gpio.h>
#include <mach/board_lge.h>//hskim0313_pull_down_test
#include <mach/vreg.h>
...
#define ESD_DETECTION_FEATURE	//LGE_DEV_PORTING GELATO_DS_E_20110414 [edward1.kim@lge.com]
#define INT_STATUS			0x14

//LGE_DEV_PORTING GELATO_DS_E_20110509 [edward1.kim@lge.com]
#define DEV_STATUS			0x13
...
struct synaptics_ts_data {
	...
	int fw_revision;
	int (*pulldown)(unsigned int on);//hskim0313_pull_down_test
};
...
static int multi = 0;
static int irq_flag = 1, melt_mode = 1, melt_count =0, tmp_oldx = 0, tmp_oldy = 0,tmp_prevx = 0, tmp_prevy = 0;// LGE_DEV_PORTING_GELATO_DS_20110622_[edward1.kim@lge.com]
...
{
	int int_mode;
#if defined(ESD_DETECTION_FEATURE)
	int device_status;
#endif
	int width0, width1;
...
//LGE_DEV_PORTING GELATO_DS_E_20110414 [edward1.kim@lge.com]

	int_mode = i2c_smbus_read_byte_data(ts->client, INT_STATUS);

#if defined(ESD_DETECTION_FEATURE)
	device_status = i2c_smbus_read_byte_data(ts->client, DEV_STATUS);
//	printk("[TOUCH] int_mode[0x14] : 0x%x  dev_status[0x13]	 : 0x%x\n",int_mode,device_status);// LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110509

	if (unlikely(int_mode < 0) ||((device_status & 0x03) == 0x03))
	{
		printk("[TOUCH]ts_reg_data. int_mode : %d  dev_status : %d  i2cfailed or esd ==> recovery touch\n",int_mode,(device_status & 0x03));
		esd_recovery(ts);
	...
	{
		printk("[TOUCH]ts_reg_data. int_mode : %d   i2cfailed ==> recovery touch\n",int_mode);
		esd_recovery(ts);
		if (ts->use_irq && irq_flag == 0) {
			irq_flag++;
			enable_irq(ts->client->irq);
...
//LGE_DEV_PORTING GELATO_DS_E_20110414 [edward1.kim@lge.com]
	if((int_mode & SYN_INT_ABS0)||(int_mode == 0x00))
	{
...
#if 0// LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110500	: for debug
		printk("[TOUCH] dev_status[0x13]:0x%x  int_status[0x14]:0x%x finger_state[0x15]:0x%x\n",ts_reg_data.dev_status,ts_reg_data.int_status,ts_reg_data.finger_state);

		printk("[TOUCH] X_high0[0x16]: 0x%x  Y_high0[0x17]:0x%x  XY_low0[0x18]:0x%x  XY_width0[0x19]:0x%x  Z_finger0[0x1A]:0x%x\n"
			    ,ts_reg_data.X_high0,ts_reg_data.Y_high0,ts_reg_data.XY_low0,ts_reg_data.XY_width0,ts_reg_data.Z_finger0);

		printk("[TOUCH] X_high1[0x1B]: 0x%x  Y_high1[0x1C]:0x%x  XY_low1[0x1D]:0x%x XY_width1[0x1E]:0x%x  Z_finger1[0x1F]:0x%x\n"
			,ts_reg_data.X_high1,ts_reg_data.Y_high1,ts_reg_data.XY_low1,ts_reg_data.XY_width1,ts_reg_data.Z_finger1);
#endif
	...
	//	printk("[TOUCH] 0x%2x  %4d  %4d\n" ,finger_status,tmp0_x,tmp0_y);
		...
		/* tapcount = 0; */
			...
			if (first_touch == 1) {
				...
				tmp_prevx = curr_ts_data.X_position[0];// LGE_DEV_PORTING_GELATO_DS_20110622_[edward1.kim@lge.com]
				tmp_prevy = curr_ts_data.Y_position[0];
			}
			else // LGE_DEV_PORTING_GELATO_DS_20110622_[edward1.kim@lge.com]
			{
				if((abs(tmp_prevx - tmp0_x) > 300) ||(abs(tmp_prevy - tmp0_y) > 300)) 
				{
					melt_count = 2;
					printk("[TOUCH] %d=====================================================================> detect ghost finger:\n",first_touch);
				}
				tmp_prevx = curr_ts_data.X_position[0];
				tmp_prevy = curr_ts_data.Y_position[0];
			}
			ts_event_touch(1, width0, curr_ts_data.X_position[0], curr_ts_data.Y_position[0], ts);
		...
		/* tapcount = 0; */
			...
			if (melt_mode == 1 && melt_count == 1) {    /* melt_mode */
				if ((abs(tmp0_x - tmp_oldx) > 140) || (abs(tmp0_y - tmp_oldy) > 140)){  /* long drag */
					if(first_touch > 6) // LGE_DEV_PORTING_GELATO_DS_20110622_[edward1.kim@lge.com]
					{
                                        //      printk("[TOUCH] 0x%2x  %4d  %4d\n" ,finger_status,tmp0_x,tmp0_y);
						i2c_smbus_write_byte_data(ts->client, SYNAPTICS_MELTING, MELTING_NO);
					//	printk("[TOUCH] Long Drag detected!! ++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
					//	printk("[TOUCH] Long Drag detected!!  0x%2x  %4d  %4d  %4d  %4d==================> set no melt %d\n",finger_status,tmp0_x,tmp_oldx,tmp0_y,tmp_oldy,first_touch);
						melt_mode = 0;
					}
				}
		...
		//		}
			...
			first_touch = 0;
			melt_count = 0; // LGE_DEV_PORTING_GELATO_DS_20110622_[edward1.kim@lge.com]
		}
...
//		printk("[TOUCH] INT_STATUS[0x14]	 : 0x%x\n",int_mode);// LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110509
//		device_status = i2c_smbus_read_byte_data(ts->client, DEV_STATUS);
//		printk("[TOUCH]bofore : do nothing !!device_status : %d\n",device_status);
//		i2c_smbus_write_byte_data(ts->client, SYN_CONTROL, 0x80);
//		printk("[TOUCH]after : do nothing !!device_status : %d\n",device_status);
 //               printk("[TOUCH]: abnormal interrupt state\n");
	}
	if (ts->use_irq && irq_flag == 0) {
		irq_flag++;
		enable_irq(ts->client->irq);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		ts->pulldown = pdata->pulldown;//hskim0313_pull_down_test
