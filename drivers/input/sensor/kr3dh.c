...
static u8 kr3dh_get_power_on_off(void)
{
	return kr3dh_misc_data->pdata->ecom_accel_pwr_state();
}
				...
				unsigned int cmd, unsigned long arg)
{
	...
	check_pwr_state = kr3dh_get_power_on_off();
	if(check_pwr_state == 0x0){ // off
		printk(KERN_INFO "[Accelerometer] %s: Power shotdown mode", __FUNCTION__);
		return -EFAULT;
	}
	// LGE_DEV_PORTING GELATO_DS_E
...
{
#if 0
	if (kr3dh_misc_data->pdata->gpio_config){
