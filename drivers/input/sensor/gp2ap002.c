...
//	u8 opmod = 0;
	...
	//LGE_DEV_PORTING GELATO_DS_S, [ks82.jung@lge.com] 2011-06-04
...
#if 0 //LGE_DEV_PORTING GELATO_DS_S, [ks82.jung@lge.com] 2011-06-04
...
#if 0 // LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110602
	if (pdev->cycle != cycle)
		pdev->cycle = cycle;
	else {
		printk(KERN_INFO "cycle is already %d\n", pdev->cycle);
...
gp2ap_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* register interrupt handler */// LGE_DEV_PORTING GELATO_DS_[edward1.kim@lge.com]_20110527_don't need both edge
	ret = request_irq(gp2ap_pdev->irq, gp2ap_irq_handler, IRQF_TRIGGER_FALLING,
