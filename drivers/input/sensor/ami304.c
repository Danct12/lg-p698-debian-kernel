...
// LGE_DEV_PORTING GELATO_DS_S [ks82.jung@lge.com] 2011-06-11	
...
// LGE_DEV_PORTING GELATO_DS_S, ks82.jung@lge.com [2011-04-26]
static u8 ami304_get_power_on_off(void)
{
	struct ecom_platform_data* ecom_pdata;

	ecom_pdata = ami304_i2c_client->dev.platform_data;
	return ecom_pdata->ecom_accel_pwr_state();
}
// LGE_DEV_PORTING GELATO_DS_E
...
static int ami304_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	// LGE_DEV_PORTING GELATO_DS_S, ks82.jung@lge.com [2011-0426]
	u8 check_pwr_state;

	check_pwr_state = ami304_get_power_on_off();
	if(check_pwr_state == 0x0){ // off
		printk(KERN_INFO "[Ecompass] %s: Power shotdown mode", __FUNCTION__);
		retval = -EFAULT;
		goto err_out;
	}
	// LGE_DEV_PORTING GELATO_DS_E
