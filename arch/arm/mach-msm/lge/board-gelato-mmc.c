...
static void sdcc_gpio_init(void)
{
	...
	int rc = 0;
//LGE_DEV_PORTING GELATO_DS_S
//LGE_UPDATE_S FILESYSTEM 2011.03.10 for Cover Detection
#ifdef CONFIG_MMC_MSM_COVER_HW_DETECTION
		if (gpio_request(GPIO_SD_DETECT_N, "sdc1_card_detect_n"))
			pr_err("failed to request gpio sdc1_card_detect_n\n");
		if (gpio_request(GPIO_MMC_COVER_DETECT, "sdc1_status_irq"))
			pr_err("failed to request gpio sdc1_status_irq\n");


		rc = GPIO_CFG(GPIO_SD_DETECT_N, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);
...
//LGE_UPDATE_S FILESYSTEM 2011.03.10 for Cover Detection
#ifdef CONFIG_MMC_MSM_COVER_HW_DETECTION
		if(!gpio_get_value(GPIO_SD_DETECT_N))
