...
static struct pt_regs     lge_kernel_regs;
				    ...
				    struct pt_regs *oldregs)
{
 ...
 int LGE_ErrorHandler_Main( int crash_side, char * message, struct pt_regs *regs)
 {
	...
	if (hidden_reset_enable) {
      ...
      //flush cache first/hard coding
       clean_and_invalidate_caches(PAGE_OFFSET, 0x1D6FFFFF, PHYS_OFFSET);

       //save ARM registers
      crash_setup_regs(&lge_kernel_regs, regs);

	if(BLUE_ERROR_HANDLER_LEVEL != 0) {
//LGE_UPDATE_S/sunkyo.park@lge.com
...
//LGE_UPDATE_E
	...
	preempt_disable();
       ...
       //LGE_UPDATE_S/sunkyo.park@lge.com/notify modem that apps enters into reset state.
...
static long errhandler_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ERRHANDLER_IOCTL_LEVEL1 :
		...
		ret = LGE_ErrorHandler_Main(ANDROID_CRASH, (char *)android_buf, NULL);
