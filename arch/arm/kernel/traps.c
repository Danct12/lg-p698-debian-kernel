...
void dump_backtrace_entry(unsigned long where, unsigned long from, unsigned long frame)
{
 ...
 // LGE_CHANGE_S [bluerti@lge.com]
	if (lge_error_analyzing == 1){
	  ...
	  if(lge_error_fb_cnt < LGE_ERROR_MAX_ROW)
	  {
	     sprint_symbol(sym1, where);
	     snprintf(crash_buf[lge_error_fb_cnt], LGE_ERROR_MAX_COLUMN, "[<%08lx>]%s", where,sym1 );
	     lge_error_fb_cnt++;
	  }
	  //LGE_UPDATE_E
	} else {	// Normal case
...
static int __die(const char *str, int err, struct thread_info *thread, struct pt_regs *regs)
{
	...
	if (!hidden_reset_enable) {
		...
		//if (!LG_ErrorHandler_enable) {
			...
			ret = LGE_ErrorHandler_Main(APPL_CRASH, (char *)crash_buf, regs);
